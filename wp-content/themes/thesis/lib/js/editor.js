...
thesis_editor = {
	...
	do_saves: function() {
		...
		thesis_editor.login_expiration();
	},
	login_expiration: function() {
		if (thesis_editor.login.templates_saved == true && thesis_editor.login.css_saved == true) {
			$('body').fadeOut(2000, function(){
				window.location = thesis_login.url;
			});
		}
		else {
			$('.t_popup_body').html('<p>Please wait while your skin state is saved.</p><p>You will be redirected momentarily&#8230;</p>');
			setTimeout(function(){
				thesis_editor.login_expiration();
			}, 500);
		}
