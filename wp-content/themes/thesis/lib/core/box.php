...
class thesis_box {
	...
	private function _class_admin() {
		...
		if (empty($this->filters['url'])) {
			...
			if (method_exists($this, 'class_options'))
	...
	public function _save_admin() {
		...
		if (!empty($_POST[$this->_class])) {
			$save = $thesis->api->set_options($this->_class_options(), $_POST[$this->_class]);
			if (empty($save))
	...
	private function _instance_admin() {
		global $thesis;
		if (!(empty($this->head) && is_array($options = $this->_options()) && !empty($options)) || (!empty($this->name) && $this->_id == $this->_class)) return;
		...
		if (!empty($_GET['canvas']) && $_GET['canvas'] == $id) {
			add_action('admin_init', array($this, '_admin_init'));
