...
class thesis_post_headline extends thesis_box {
	...
	protected function html_options() {
		...
		$html = $thesis->api->html_options(array(
			...
			'p' => 'p',
			'span' => 'span'), 'h1');
		$html['class']['tooltip'] = sprintf(__('This box already contains a %1$s called <code>headline</code>. If you wish to add an additional %1$s, you can do that here. Separate multiple %1$ses with spaces.%2$s', 'thesis'), $thesis->api->base['class'], $thesis->api->strings['class_note']);
		unset($html['id']);
		return array_merge($html, array(
			'link' => array(
				'type' => 'checkbox',
	...
	public function html($args = array()) {
		...
		echo
			...
			'<span class="post_author'. (!empty($this->options['class']) ?
				' '. $thesis->api->esc($this->options['class']) : ''). '"'. (!empty($schema) ?
...
class thesis_comments_nav extends thesis_box {
	...
	public function html($args = array()) {
		...
		echo
			"$tab<div class=\"comment_nav", (!empty($this->options['class']) ? ' '. $thesis->api->esc($this->options['class']) : ''), "\">\n",
			(!empty($previous_link) ?
...
class thesis_comment_date extends thesis_box {
	...
	public function html($args = array()) {
		...
		echo
			str_repeat("\t", !empty($depth) ? $depth : 0),
			'<span class="comment_date', (!empty($this->options['class']) ? ' '. $thesis->api->esc($this->options['class']) : ''), '">',
