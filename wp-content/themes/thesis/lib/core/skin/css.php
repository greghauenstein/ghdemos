...
class thesis_css {
	...
	public function admin() {
		...
		echo
			"\t\t<h3>", sprintf(__('Custom %s', 'thesis'), $thesis->api->base['css']), "</h3>\n",
	...
	public function canvas_head() {
		echo
			...
			$this->update(stripslashes($this->css), stripslashes($this->custom), true),
			"\n</style>\n";
	}
	...
	public function write($skin, $custom) {
		$css = strip_tags($this->update(apply_filters('thesis_css', $this->reset(). $skin, $this->reset(), $skin), !empty($custom) ? "/*---:[ custom CSS ]:---*/\n". $custom : false));
		if (is_multisite()) {
